...
find ${LOGDIR} -name "06_iperf_udp*.log" | sort | xargs grep " ms\|WARNING: did not" | awk 'BEGIN { seq = 0 }{printf("%5d %s %10s %10s %10s %10s %20s %-15s\n", seq, substr($1,25,12), $7, $8, $9, $10, $11, $12); seq++}'
...
LOGLIST=`find ${LOGDIR} -name "06_ab_to_local*.log" | sort`
...
LOGLIST=`find ${LOGDIR} -name "06_ab_to_ext*.log" | sort`
for LOGFILE in $LOGLIST; do
  LOGDATE=${LOGFILE:24:12}
  RPS_=`grep "Requests per second" $LOGFILE | awk '{printf("%10s", $4);}'`
  Rate=`grep "Transfer rate" $LOGFILE | awk '{printf("%10s", $3);}'`
