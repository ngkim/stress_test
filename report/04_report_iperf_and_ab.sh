...
find ${LOGDIR} -name "04_iperf_udp*.log" | sort | xargs grep " ms\|WARNING: did not" | awk 'BEGIN { seq = 0 }{printf("%5d %s %10s %10s %10s %10s %20s %-15s\n", seq, substr($1,25,12), $7, $8, $9, $10, $11, $12); seq++}'
...
LOGLIST=`find ${LOGDIR} -name "04_ab_*.log" | sort`
